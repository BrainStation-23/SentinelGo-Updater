...
func ensureHomeDirectory() (string, error) {
	// Strategy 1: Check USERPROFILE environment variable (Windows standard)
	if home := os.Getenv("USERPROFILE"); home != "" {
		LogInfo("Home directory detected from USERPROFILE environment variable: %s", home)
		return home, nil
	}
...
func getPossibleBinaryPaths() []string {
	...
	// Method 2: Check USERPROFILE
	if userProfile := os.Getenv("USERPROFILE"); userProfile != "" {
		possiblePaths = append(possiblePaths, filepath.Join(userProfile, "go", "bin", "sentinel.exe"))
	}
